...
object mydataload {
 def addValue (line:String) : String = {
   ...
   for (i <- 0 to (elements.length - 1 )){
     ...
     if (elements(i).indexOf("io:xyz") > 0 ){
        var st = elements(i).indexOf("io:xyz")
        var en = elements(i).indexOf(":{\"val")
        var results =  elements(i).substring((st),en-1)
        outputstr  = outputstr + "-" + results 
     }
 ...
 def main(args: Array[String]) {
    ...
    //val ratings = data.map( x => x.split('\t') ).map( x => Rating(x(0).toInt, x(1).toInt, x(2).toDouble) ).cache()
     ...
     // ratings.collect().foreach(a => println(a))
      ...
      val lexRDD = ratings.map(x => x.split(',')).map(fields => (fields(1),fields(2))).flatMapValues(fields => fields.split('-'))
 
      lexRDD.collect().foreach(b => println(b))
